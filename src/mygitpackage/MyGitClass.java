package mygitpackage;

public class MyGitClass {
	
	public static void main(String[] args) {
		
		System.out.println("Version 1");
		
		System.out.println("My second commit");
		
		System.out.println("My third commit");
		
        System.out.println("My fourth commit");

		System.out.println("Uzak sunucudaki kodum");

		
		System.out.println("Uzak sunucudaki YENÄ° kodum");
		
		
		System.out.println("My fourty commit");
		
		
		System.out.println("24. satÄ±rda uzak sunucu");
	
		System.out.println("Elif branchinde ilk commit 26. satýr");
		
		System.out.println("Elif branch kaydet artýk 28. satýr");
		
		System.out.println("Elif branch deneme tekrar 30.satýr");
		
		
		
		
	
	}

}
